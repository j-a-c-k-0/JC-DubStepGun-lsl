...
startup()
{
list_inv(INVENTORY_NOTECARD);
list_inv(INVENTORY_SOUND);
...
slist_size = llGetListLength(songlist); llRequestPermissions(llGetOwner(),PERMISSION_TAKE_CONTROLS);
...
list_inv(integer itype)
{
    integer count = llGetInventoryNumber(itype); string ItemName;
    while (count--)
    {
    ItemName = llGetInventoryName(itype, count);
    if (ItemName != llGetScriptName())  
    if(ItemName==ignore_notecard){ }else{songlist += ItemName;}
    }return;
}
...
"\n"+"[ Status ]"+"\n\n"+
"Memory Notecard = "+llList2String(targe,0)+"\n"+
